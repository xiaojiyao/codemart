<template>
  <div class="app-container">
    <el-table :data="dataList.filter(data => data.isStar)" style="width: 100%">
      ...
      <el-table-column width="250px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="danger" @click="deleteStar(scope.row.starId)">取消收藏</el-button>
        </template>
      </el-table-column>
          ...
          if (res.data.msg == 'success') {
            ...
            this.dataList.forEach(element => {
              ...
              this.$http.post('/getStarList', condition).then(res => {
                if (res.data.msg == 'success') {
                  element.isStar = !!res.data.pageData.length
                  element.starId = res.data.pageData[0].id
                }
              })
              this.$http.post('/getBidList', condition1).then(res => {
                if (res.data.msg == 'success') {
                  this.$set(element, 'isBid', !!!res.data.pageData.length)
                  element.bidId = res.data.pageData[0].id
                  element.bidStatus = res.data.pageData[0].status
                }
              })
            });
          }
