  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogEditFormVisible">
      <el-form label-position="left" label-width="100px" style="width: 500px; margin-left:50px;" ref="editForm" :rules="editRules"
        ...
        <el-form-item label="开发类型" prop="development_type">
          <el-select v-model="editForm.development_type" placeholder="请选择类型">
            <el-option :label="tag" :value="tag" :key="tag" v-for="tag in projectData"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="招募类型" prop="recruitment_type">
          <el-select v-model="editForm.recruitment_type" placeholder="请选择类型">
        ...
        <el-form-item label="开发者类型" prop="development_name">
          <el-select v-model="editForm.development_name" multiple placeholder="请选择类型" style="width:400px">
            <el-option :label="tag" :value="tag" :key="tag" v-for="tag in developerData"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="起讫时间" prop="time">
          <el-date-picker v-model="editForm.time" type="datetimerange" :picker-options="pickerOptions2" range-separator="至"
            start-placeholder="开始日期" end-placeholder="结束日期" align="right">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="项目附件">
          <span style="cursor: pointer;margin-right:15px;" @click="openWindow(editForm.enclosure)" v-if="editForm.enclosure">{{editForm.enclosure}}</span>
          <el-button type="primary" size="small" @click="uploadAddFile('editFile')">上传附件</el-button>
          <input type="file" v-show="false" ref="editFile" @change="uploadAddFileChange('editFile',editForm)">
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="updateProject" type="primary">确认</el-button>
      </div>
    </el-dialog>
    <el-dialog :visible.sync="dialogFormVisible">
      <el-form label-position="left" label-width="100px" style="width: 500px; margin-left:50px;">
        ...
        <el-form-item label="起讫时间" prop="time">
          <span>{{new Date(currentForm.create_time).toLocaleString() + ' - ' + new
            Date(currentForm.update_time).toLocaleString()}}</span>
        </el-form-item>
        <el-form-item label="上传附件">
          <span style="cursor: pointer;margin-right:15px;" @click="openWindow(currentForm.enclosure)" v-if="currentForm.enclosure">{{currentForm.enclosure}}</span>
        </el-form-item>
        <el-form-item label="联系QQ">
      ...
      var checkPrice = (rule, value, callback) => {
        if (!value) {
          ...
          if (reg.test(value)) {
            ...
            return callback(new Error("请输入正确价格格式"));
          }
      ...
      getDynamicTags() {
        this.$http.post('/getLabelList').then(res => {
          if (res.data.msg == 'success') {
            this.developerData = res.data.pageData.filter(e => e.type == "developer")[0].label.split(',')
            this.projectData = res.data.pageData.filter(e => e.type == "project")[0].label.split(',')
          }
        })
      },
      getProjectList() {
