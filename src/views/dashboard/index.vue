  ...
  <div class="body">
    <div class="dashboard-container">
      <el-menu
        class="el-menu-demo"
        mode="horizontal"
        style="position: fixed;top: 0;width: 100%;z-index: 9999999;"
      >
      ...
      <el-card class="box-card">
        <div class="search-box">
          ...
          <div>
            <span
              :class="'search-value ' + (activeItem == item ? 'active' : '')"
        ...
        <div class="search-box">
          ...
          <div>
            <span
              :class="'search-value ' + (activeItem1 == item ? 'active' : '')"
              v-for="item in dynamicTags2"
              :key="item"
              @click="activeItem1 = item"
            >{{item}}</span>
          </div>
        </div>
        <div class="search-box">
          ...
          <div>
            <span
              :class="'search-value ' + (activeItem2 == item ? 'active' : '')"
              v-for="item in dynamicTags"
              :key="item"
              @click="activeItem2 = item"
            >{{item}}</span>
          </div>
        </div>
      </el-card>
      ...
      >
        ...
        >
          ...
          >
            ...
            <el-tag
              size="small"
              v-if="product.active === '开发中'"
            >开发中</el-tag>
            <el-tag
              type="success"
              size="small"
              v-if="product.active === '已结束'"
            >已结束</el-tag>
            <el-tag
              type="info"
              size="small"
              v-if="product.active === '招募中'"
            >招募中</el-tag>
          </div>
    ...
    getDynamicTags() {
      this.$http.post("/getLabelList").then(res => {
        if (res.data.msg == "success") {
          this.dynamicTags = res.data.pageData
            .filter(e => e.type == "developer")[0]
            .label.split(",");
          this.dynamicTags1 = res.data.pageData
            .filter(e => e.type == "project")[0]
            .label.split(",");
      ...
      this.$http.post("/getProjectList").then(res => {
        if (res.data.msg == "success") {
          ...
          this.productList.forEach(product => {
            const day = Math.ceil(
              (new Date(product.update_time).getTime() -
                new Date(product.create_time).getTime()) /
                (60 * 60 * 60 * 1000)
            );
    ...
    activeItem(newV, oldV) {
      if (newV !== "全部") {
        if (this.activeItem1 == "全部" && this.activeItem2 == "全部") {
          this.productList = this.productList1.filter(
            product => product.development_type == newV
          );
        } else {
          if (this.activeItem1 == "全部") {
            this.productList = this.productList1.filter(
              product =>
                product.development_name.includes(this.activeItem2) &&
          ...
          if (this.activeItem2 == "全部") {
            this.productList = this.productList1.filter(
              product =>
                product.active == this.activeItem1 &&
                product.development_type == newV
            );
          }
          if (this.activeItem1 != "全部" && this.activeItem2 != "全部") {
        ...
        if (this.activeItem1 == "全部" && this.activeItem2 == "全部") {
          ...
          if (this.activeItem1 == "全部") {
            this.productList = this.productList1.filter(product =>
              product.development_name.includes(this.activeItem2)
            );
          }
          if (this.activeItem2 == "全部") {
            this.productList = this.productList1.filter(
              product => product.active == this.activeItem1
            );
          }
          if (this.activeItem1 != "全部" && this.activeItem2 != "全部") {
            this.productList = this.productList1.filter(
      ...
      if (newV !== "全部") {
        if (this.activeItem == "全部" && this.activeItem2 == "全部") {
          ...
          if (this.activeItem2 == "全部") {
            this.productList = this.productList1.filter(
              product =>
                product.development_type == this.activeItem &&
                product.active == newV
            );
          }
          if (this.activeItem != "全部" && this.activeItem2 != "全部") {
        ...
        if (this.activeItem == "全部" && this.activeItem2 == "全部") {
          ...
          if (this.activeItem2 == "全部") {
            this.productList = this.productList1.filter(
              product => product.development_type == this.activeItem
            );
          }
          if (this.activeItem != "全部" && this.activeItem2 != "全部") {
            this.productList = this.productList1.filter(
