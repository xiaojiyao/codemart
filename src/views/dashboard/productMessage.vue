      ...
      <div class="box-card">
        <div class="box-header">
          ...
          <div class="segmentation">
            ...
            <span
              class="gray"
              style="transform: translateY(-2px);display: inline-block;margin:0 5px;"
            >|</span>
            <span class="gray">
              类型
            </span>
            ...
            <span
              class="gray"
              style="float:right"
            >报名人数：{{productMessage.contendCount}}</span>
          </div>
        </div>
          ...
          <div class="file">
            ...
            <template v-else>
              <svg-icon
                ...
                style="color:#4289DB;margin-left:10px;cursor: pointer;"
                @click="download(productMessage.enclosure)"
              />
              <span
                style="color: #979FA8;font-size:14px;cursor: pointer;"
                @click="download(productMessage.enclosure)"
              >{{productMessage.enclosure}}</span>
              <span
                style="float:right;color: #979FA8;font-size:14px;cursor: pointer;"
                @click="download(productMessage.enclosure)"
              >下载</span>
            </template>
          </div>
    ...
    getProductMessage() {
      this.$http.post("/getProjectList").then(res => {
        if (res.data.msg == "success") {
          this.productMessage = res.data.pageData.filter(
            product => product.id == this.$route.query.id
          )[0];
          const day = Math.ceil(
            (new Date(this.productMessage.update_time).getTime() -
              new Date(this.productMessage.create_time).getTime()) /
              (60 * 60 * 60 * 1000)
          );
